;;; =================================
;;;   ABUBAKAR AWUMBILA ( DEVELOPED UNDER SUPERVISION OF AND WITH HELP FROM LUKE HUNSBERGER FOR HIS ARTIFCIAL INTELLIGENCE CLASS )
;;;   MINIMAX IMPLEMENTATION OF CHESS GAME WITH ALPHA-BET PRUNING
;;;   FALL 2010
;;; =================================
...
(defconstant *won-game* 1000000)
(defconstant *lost-game* -1000000)

;;; alpha and beta values

(defconstant *alpha* -1000000000)
(defconstant *beta* 1000000000)
...
(defconstant *pawn-val* 100)
(defconstant *knight-val* 325)
(defconstant *bishop-val* 350)
(defconstant *rook-val* 500)
(defconstant *queen-val* 975)
(defconstant *king-val* 100000)
...
;;; DEPTH-CUTOFF: specifies the depth at which the static evaluation function should
;;;               be used to cut off further search

(defparameter *cutoff-depth* 4)

;;; *NUM-MOVES-TRIED*: the number of moves COMPUTE-MAX and COMPUTE-MIN have trie

(defparameter *num-moves-tried* 0)

;;; *BEST-MOVE*: the best move possible
...
;;; GET-PIECE-VALUE
;;; -------------------------------------------------------
;;; INPUT: type, a piece type
;;; OUTPUT: the value of the type

(defun get-piece-value (type)
    (cond
     ((= type *pawn*)   *pawn-val*)
     ((= type *knight*) *knight-val*)
     ((= type *bishop*) *bishop-val*)
     ((= type *rook*)   *rook-val*)
     ((= type *queen*)  *queen-val*)
     ((= type *king*)   *king-val*)
     (T                 (format t "Wrong piece type!")))
    )
...
;;;   EVAL SUBTOTAL -- the points each player has according to its live pieces
...
(defun print-chess (game str depth)
  ...
  (let* ((bored (chess-board game))
	 ...
	 (scores (chess-eval-subtotal game))
	 (white-score (aref scores *white*))
	 (black-score (aref scores *black*)))
    ...
    (format t "White's score: ~A,   Black's score: ~A~%" white-score black-score)
...
(defun pull-piece! (game pc)
  (let ((subtotals (chess-eval-subtotal game))
	(owner (piece-owner pc))) 
    (setf (piece-live? pc) nil)
    (setf (aref (chess-board game) (piece-row pc) (piece-col pc)) 
      nil)
    (setf (aref subtotals owner)
      (- (aref subtotals owner)
	 (piece-value pc))))
  )
...
;;;    Increments the subtotal value of the player
...
(defun init-game ()
  (let ((game (make-chess)))
    ;; Create/Set pawns
    (dotimes (i 8)
      (create-set-new-piece! game *white* *pawn* 1 i i)
      (create-set-new-piece! game *black* *pawn* 6 i i))
    ;; Create/Set rooks
    (create-set-new-piece! game *white* *rook* 0 0 8)
    (create-set-new-piece! game *black* *rook* 7 0 8)
    (create-set-new-piece! game *white* *rook* 0 7 9)
    (create-set-new-piece! game *black* *rook* 7 7 9)
    ;; Create/Set knights
    (create-set-new-piece! game *white* *knight* 0 1 10)
    (create-set-new-piece! game *black* *knight* 7 1 10)
    (create-set-new-piece! game *white* *knight* 0 6 11)
    (create-set-new-piece! game *black* *knight* 7 6 11)
    ;; Create/Set bishops
    (create-set-new-piece! game *white* *bishop* 0 2 12)
    (create-set-new-piece! game *black* *bishop* 7 2 12)
    (create-set-new-piece! game *white* *bishop* 0 5 13)
    (create-set-new-piece! game *black* *bishop* 7 5 13)
    ;; Create/Set kings/queens
    (create-set-new-piece! game *white* *king* 0 4 14)
    (create-set-new-piece! game *black* *king* 7 4 14)
    (create-set-new-piece! game *white* *queen* 0 3 15)
    (create-set-new-piece! game *black* *queen* 7 3 15)
    ;; RETURN THE GAME!
    game))
...
(defun do-move! (game check-legal? r1 c1 r2 c2)
  (cond
   ...
   ;; Case 2: The game is over. Make no moves
   ((game-over? game)
    (format t "Can't you see the king is dead? GAME OVER!~%"))
   
   ;; Case 3:  Legal Move (or wasn't asked to check)
   (t
    (let* ((bored (chess-board game))
...
;;;  LEGAL-MOVES
;;; ------------------------------------------------------
;;;  Fetch legal moves for all the LIVE pieces of whoever's turn it is.
...
(defun compute-move (g)
  ...
  (setf *best-move* NIL)
  ;; initialize the number of moves tried
  (format t "The root's alpha value is: ~A~%" (compute-max g 0 *alpha* *beta* #'eval-func))
  (format t "The number of moves tried before reaching the best move: ~A~%" *num-moves-tried*)
  ;; initialize *NUM-MOVES-TRIED*
  (setf *num-moves-tried* 0)
  *best-move*)
...
;;; EVAL-FUNC
;;; ----------------------------------------------------------------
;;; INPUT: GAME, a chess game
;;; OUTPUT: The value of all the pieces in the game, with the player's
;;;         pieces being positive and the opponents pieces being negative.

(defun eval-func (game)
  (let* (;; PLAYER: the current player
	 (player (chess-whose-turn? game))
	 ;; SUBTOTALS: the scores vector
	 (subtotals (chess-eval-subtotal game))
	 ;; WHITE-SCORE: the score of the white player
	 (white-score (aref subtotals *white*))
	 ;; BLACK-SCORE: the score of the black player
	 (black-score (aref subtotals *black*)))
    
    ;; if the current player is white
    (if (= player *white*)
	;; add the white pieces score to the negative black pieces score
	(+ white-score
	   (* -1 black-score))
      ;; otherwise, if player is black,
      ;; add the black pieces scores to the negative white pieces score
      (+ black-score
	 (* -1 white-score)))
    )
  )
...
;;; ----------------------------------------------------------------
;;; INPUT: GAME, a chess game
;;;        CURR-DEPTH, the current depth of the search
;;;        ALPHA, the alpha value
;;;        BETA, the beta value
;;;        EVAL-FUNCY, the evaluation function
;;; OUTPUT: if CURR-DEPTH is 0, return the best move found
;;;         also returns the ALPHA value.
;;; SIDE-EFFECT: changes *BEST-MOVE* whenever alpha 

(defun compute-max (game curr-depth alpha beta eval-funcy)
  (let* ()
    (cond
     ;; if  the game is over.
     ;; Return the value for LOSING the game - 
     ;; (in COMPUTE-MIN, this will be WINNING the game)
     ((game-over? game)
      *lost-game*)
     
     ;; if CURR-DEPTH has reached *CUTOFF-DEPTH*,
     ;; call static evaluation function
     ((= curr-depth *cutoff-depth*)
      (return-from compute-max (funcall eval-funcy game)))
     
     ;; otherwise...
     (T
      (let* (;; LEG-MOVES: a list of legal moves
	     (leg-moves (legal-moves game)))
	
	;; while there are still legal moves to try...
	(when (not (null leg-moves))
	  ;; go through the list of legal move
	  (dolist (curr-move leg-moves)
	    ;; do the move
	    (apply #'do-move! game NIL curr-move)
	    
	    ;;increment the number of moves tried
	    (incf *num-moves-tried*)
	    
	    ;; make recursive call to COMPUTE-MIN with appropriate input (???)
	    (let* (;; the new BETA-VALUE after COMPUTE-MIN
		   (new-alpha (compute-min game (+ 1 curr-depth) alpha beta eval-funcy)))
	      
	      ;; undo the move DESTRUCTIVELY!!!
	      (undo-move! game)
	      
	      ;;(format t "alpha: ~A, new alpha: ~A ~%"alpha new-alpha)
	    
	      ;; Check if the ALPHA value needs updating
	      (when (> new-alpha alpha)
		;; set ALPHA to NEW-ALPHA
		(setf alpha new-alpha)
		;; update the BEST-MOVE if CURR-DEPTH is 0
		(when (= 0 curr-depth)
		  (setf *best-move* curr-move))
		  
	   
		;; Since we've updated alpha, we can now check for pruning.
		;; in case of pruning, we use the RETURN-FROM special form
		;; So, if beta is smaller or equal to alpha, prune.
		;; Meaning, return NEW-BETA
		(when (<= beta alpha)
    ...
    ;; at the very end, return the very best ALPHA
    alpha
    )
  )
...
;;;        BETA, the beta value
;;;        EVAL-FUNCY, the evaluation function
;;; OUTPUT: returns the best BETA VALUE

(defun compute-min  (game curr-depth alpha beta eval-funcy)
  (let* ()
    (cond
     ;; if  the game is over.
     ;; Return the value for LOSING the game - 
     ...
     ;; call static evaluation function
     ((= curr-depth *cutoff-depth*)
      ;;(funcall eval-funcy game))
      (return-from compute-min (funcall eval-funcy game))
      )
	...
	(dolist (curr-move leg-moves)
	  ...
	  ;; make recursive call to COMPUTE-MIN with appropriate input (???)
	  (let* (;; the new BETA-VALUE after COMPUTE-MIN
		 (new-beta (compute-max game (+ 1 curr-depth) alpha beta eval-funcy)))
	    
	    ;; undo the move DESTRUCTIVELY!!!
	    (undo-move! game)
	    
	    ;;(format t "beta: ~A, new beta: ~A ~%"beta new-beta)
	    
	    ;; Check if the ALPHA value needs updating
	    (when (< new-beta beta)
	      ;; set BETA to NEW-BETA
	      (setf beta new-beta)
    ...
    ;; at the very end, return the very best BETA
    beta
    )
  )
...
;;; --------------------------------------------------------------------
;;; INPUT: GAME, a chess game
;;;         NUM-MOVES, the number of random moves we want to perform
;;; OUTPUT: NONE
;;; SIDE-EFFECTS: applies a certain number of random moves to a game


(defun rand-moves (game num-moves)
  ;; do a random moves NUM-MOVES times
    (dotimes (i num-moves)
      (let* (;; LEG-MOVEs: a list of all possible legal moves
	     (moves-list (legal-moves game)))
	;; if there are still legal moves,
	;; choose a random one out of the list and do it
	(when (not (null moves-list))
	  (let (;; a random move out of the legal-moves list
		(move (nth (random (length moves-list))
			   moves-list)))
...
;;; --------------------------------------------------------------------
;;; INPUT: GAME, a chess game
;;;        NUM-TIMES, number of times to perform COMPUTE-MAX
;;; OUTPUT: NONE
;;; SIDE-EFFECTS: computes COMPUTE-MAX on GAME NUM-TIMES times
;;;               and displays the results


;;; compute-max many times and apply the moves the come back to see if they are legal
(defun test-comp-move     (game num-times)
  (dotimes (i num-times)
    (cond
     ;; while the game is not over,
     ((not (game-over? game))
      ;; compute the best move - *BEST-MOVE* is stored, so there's no need
      ;; to use the output of COMPUTE-MOVE
      (compute-move game)
      ;; apply *BEST-MOVE* on GAME
      (apply #'do-move! game T *best-move*)
      ;; print the chess game after the move
      (print-chess game t 0))
     ;; game is over...
     (t
...
;; create a chess game and perform 40 random move
